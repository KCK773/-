...
{
	int err = bind(mSocket, &inBindAddress.mScokAddr, inBindAddress.GetSize());		//소켓이 어떤 주소와 포트를 직접 바인딩하려면 bind()함수를 호출해야한다.
...
{
	int byteSentCount = sendto(mSocket, static_cast<const char*>(inData), inLen, 0, &inTo.mScokAddr, inTo.GetSize()); //데이터를 보낼때 seinto() 함수 사용
